...
contract UniswapTwapPriceOracleRoot {
    ...
    function price0TWAP(address pair) internal view returns (uint) {
        ...
        return (currPx0Cumu - lastObservation.priceCumulative) / (now - lastObservation.timestamp); // overflow is desired
    }
    ...
    function price1TWAP(address pair) internal view returns (uint) {
        ...
        return (currPx1Cumu - lastObservation.priceCumulative) / (now - lastObservation.timestamp); // overflow is desired
    }
