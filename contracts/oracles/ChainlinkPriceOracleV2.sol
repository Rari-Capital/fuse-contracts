...
contract ChainlinkPriceOracleV2 is PriceOracle, BasePriceOracle {
    ...
    function _price(address underlying) internal view returns (uint) {
        ...
        if (baseCurrency == FeedBaseCurrency.ETH) {
            ...
            return tokenEthPrice >= 0 ? uint256(tokenEthPrice).mul(1e18).div(10 ** feed.decimals()) : 0;
            ...
            return tokenUsdPrice >= 0 ? uint256(tokenUsdPrice).mul(1e26).div(10 ** feed.decimals()).div(uint256(ethUsdPrice)) : 0;
            ...
            return tokenBtcPrice >= 0 ? uint256(tokenBtcPrice).mul(uint256(btcEthPrice)).div(10 ** feed.decimals()) : 0;
        }
    }
